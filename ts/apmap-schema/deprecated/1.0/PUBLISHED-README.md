<!--
  ARCHIVED. This is the APMap 1.0 specification exactly as it was published at
  $MAPPER_ROOT/formats/apmap/1.0/README.md, preserved verbatim below when PREPROD-01C
  removed that mirror.

  It is HISTORY, not the contract. The normative rule catalogue is SEMANTICS.md in this
  package, which succeeded it; the machine-readable contracts are schema/apmap-1.1.schema.json
  (current) and schema/deprecated/apmap-1.0.schema.json (a supported legacy READ contract).

  Kept because the mirror held exactly one file AULIBS did not already carry byte-for-byte,
  and deleting the only copy of a specification is not the same act as retiring a duplicate.
-->
